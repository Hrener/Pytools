...
from data.voc0712 import VOCAnnotationTransform, VOCDetection, BaseTransform, VOC_CLASSES, CLASS_COLOR
import numpy as np
import cv2
import time
from decimal import *
...
parser.add_argument('--trained_model', default='weights/yolo_best_model.pth',
...
def detect_net(net, device, testset, transform, thresh, mode='voc'):
    ...
    for index in range(num_images):
        ...
        detections = y
        print("detection time used ", Decimal(time.time()) - Decimal(t0), "s")
...
def detect():
    ...
    testset = VOCDetection(args.voc_root, [('2007', 'test')], BaseTransform(input_size, mean=(0.406, 0.456, 0.485), std=(0.225, 0.224, 0.229)), VOCAnnotationTransform())
