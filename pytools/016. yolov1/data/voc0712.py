...
def gt_creator(input_size, stride, label_lists=None):
    ...
    w_grid = w // stride
    h_grid = h // stride
    # [1 + 1 + 4 + 1]：[odj_conf, gt_class_id, tx, ty, tw, th, weight]
    gt_tensor = np.zeros([batch_size, h_grid, w_grid, 1 + 1 + 4 + 1])

    # generate gt whose style is yolo-v1
    for batch_index in range(batch_size):
        for gt_label in label_lists[batch_index]:
            gt_class = int(gt_label[-1])
            result = generate_dxdywh(gt_label, w, h, stride)
            # 在gt_tensor中有物体的位置均赋以gt值，否则为零
...
class BaseTransform:
    ...
    def base_transform(self, image):
        x = cv2.resize(image, (self.size[1], self.size[0])).astype(np.float32)
        x /= 255.
        x -= self.mean
        x /= self.std
        return x
